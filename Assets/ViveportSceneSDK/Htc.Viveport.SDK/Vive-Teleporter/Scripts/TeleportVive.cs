...
public class TeleportVive : MonoBehaviour {
    ...
    [Tooltip("Input Abstraction that allows for differing behavior based on the active controller hardware")]
    public TeleportInput TeleportInput;
    ...
    {
        get { return NavmeshAnimator; }
        set { NavmeshAnimator = value; }
    }
	...
	void Update ()
    {
        ...
        {
            bool shouldTeleport, shouldCancel;
            TeleportInput.GetSelectionStatus(out shouldTeleport, out shouldCancel);
            if (shouldTeleport || shouldCancel)
            {
                ...
                TeleportInput.ExitSelectionMode();
            ...
            } else
            {
                ...
                if (IsClicking && Mathf.Abs(angleClickDiff) > HapticClickAngleStep)
                {
                    ...
                    if (Pointer.PointOnNavMesh)
                        TeleportInput.TriggerHapticPulse();
                }
                ...
                {
                    IsClicking = true;
                    TeleportInput.TriggerHapticPulse(750);
                    LastClickAngle = Pointer.CurrentPointVector;
                }
        ...
        {
            Transform trackedParent;
            if(!TeleportInput.TryEnterSelectionMode(out trackedParent)) return;
            
            Pointer.transform.parent = trackedParent;
    ...
    public void ManualTeleport(Vector3 worldPoint)
    {
        Pointer.SelectedPoint = worldPoint;
    ...
    public void Hide(bool shouldCancel)
    {
        if(shouldCancel) CancelTeleport();
        _isHidden = true;
    }

    public void Show(bool shouldCancel)
    {
        if(shouldCancel) CancelTeleport();
        _isHidden = false;
    }
